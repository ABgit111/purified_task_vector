...
parser.add_argument("--language_model_name", type=str, default="deberta", help="name of the language model", choices=["gpt2","roberta-base","deberta"])
parser.add_argument("--merging_method_name", type=str, default="emr_merging", help="name of the method to merge models",
                    choices=["pave_emr_merging"])
parser.add_argument("--batch_size", type=int, default=64, help="batch size")
parser.add_argument("--gpu", type=int, default=0, help="number of gpu to use")
parser.add_argument("--num_ada_examples", type=int, nargs="+", help="numbers of examples to compute regmean weights")
...
def get_emr_merge_performance(args: argparse.Namespace, models_to_merge: list, trainers: list, logger: logging.Logger,
                          merging_method: MergingMethod, tokenizer: transformers.AutoTokenizer, num_ada_examples: list, reduce_non_diagonal_ratio: float):
    ...
    Vector_unified, masks, rescales = merging_method.get_pave_emr_merged_model(merged_model=merged_model,
    ...
    for idx, (dataset_name, model_to_merge, trainer) in enumerate(zip(args.dataset_names, models_to_merge, trainers)):
        ...
        with torch.no_grad():
            device = merged_model.device  # explicitly get the device of the merged model
            merged_params = {}
            for param_name in task_vector_recon:
                merged_params[param_name] = pretrained_param_dict[param_name].to(device) + task_vector_recon[param_name].to(device)
...
if __name__ == "__main__":
    ...
    # Create a trainer for the pretrained model
    # We'll use the first dataset's data for the pretrained model trainer
    first_dataset_name = args.dataset_names[0]
    ...
    pretrained_training_args = TrainingArguments(
        output_dir=f"./ckpts/deberta/deberta-base/{args.language_model_name}",
        per_device_train_batch_size=args.batch_size,
        per_device_eval_batch_size=args.batch_size,
    )
    
    pretrained_trainer = CustomizedTrainer(
        model=pretrained_model.to('cpu'),
        args=pretrained_training_args,
        train_dataset=train_dataset,
        eval_dataset=test_dataset,
        compute_metrics=partial(compute_metrics, dataset_names=[first_dataset_name]),
        tokenizer=tokenizer
    )
    
    # Add pretrained model to models_to_merge list
    models_to_merge.append(pretrained_model.to('cpu'))
    # Add pretrained trainer to trainers list
    trainers.append(pretrained_trainer)
    ...
    performance = get_emr_merge_performance(args=args, models_to_merge=models_to_merge, trainers=trainers, logger=logger, merging_method=merging_method, tokenizer=tokenizer, num_ada_examples=args.num_ada_examples, reduce_non_diagonal_ratio=args.reduce_non_diagonal_ratio)
