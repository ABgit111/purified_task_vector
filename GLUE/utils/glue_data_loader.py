...
class GLUEDataLoader:
    def __init__(self, tokenizer: transformers.AutoTokenizer, matched_val: bool = True):
        """
    ...
    def load_dataset(self, dataset_name: str, train_split_ratio_for_val: float = 0.1, max_seq_length: int = 128):
        ...
        :return:
        """
        base_path = '/ibex/user/anb/Model_merging/EMR_Merging-main/merge_lm/GLUE_dataset/' + dataset_name
        
        # Load datasets from CSV files using pandas
        train_df = pd.read_csv(os.path.join(base_path, 'train.csv'))
        if dataset_name == 'mnli' and self.matched_val:
            val_df = pd.read_csv(os.path.join(base_path, 'validation_matched.csv'))
        elif dataset_name == 'mnli' and not self.matched_val:
            val_df = pd.read_csv(os.path.join(base_path, 'validation_mismatched.csv'))
        else:
        ...
        if dataset_name == 'mnli':
            dataset_dict['validation_matched'] = val_data
