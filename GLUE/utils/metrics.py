...
def compute_metrics(eval_pred: EvalPrediction, dataset_names: list):
    ...
    if len(dataset_names) > 1:
        ...
        results = []
        # np.ndarray
        dataset_ids = np.asarray(eval_pred.label_ids[0])
        for dataset_id in np.unique(dataset_ids):
            ...
            if single_dataset_num_labels > 1:
                ...
                labels = np.asarray(eval_pred.label_ids[1][single_dataset_indices], dtype=np.longlong)
            else:
                # regression task
                assert single_dataset_num_labels == 1, "wrong number of labels!"
                predictions = np.asarray(eval_pred.predictions[single_dataset_indices][:, 0])
                labels = np.asarray(eval_pred.label_ids[1][single_dataset_indices])
    ...
    # single task setting
        ...
        if eval_pred.predictions.shape[1] > 1:
            ...
            predictions = np.asarray(eval_pred.predictions).squeeze(axis=1)
        # load the metric function from huggingface or cache
        metric_func = evaluate.load("glue", config_name=dataset_name)

        result = metric_func.compute(predictions=predictions, references=np.asarray(eval_pred.label_ids))
        if len(result.keys()) > 1:
            # .item() -> convert numpy.float64 to float
            result["averaged_scores"] = np.mean(list(result.values())).item()
        return result
