...
from model_merging_methods.task_vector import TaskVector, Ada_TaskVector_Cholesky
...
from model_merging_methods.rank_strategy import rank_search


class MergingMethod:
    def __init__(self, merging_method_name: str, model_name: str):
        """
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
                       ...
                       nums_regmean_examples: list = None, rank_scale: list = None, reduce_non_diagonal_ratio: float = 1.0, model_name: str = None, param_value_mask_rate: float = 0.8,
                       weight_format: str = "delta_weight", weight_mask_rates: list = None, use_weight_rescale: bool = True, mask_strategy: str = "random",
                       mask_apply_method: str = "average_merging", models_use_deepcopy: bool = False, num_ada_examples: list = None, regularization: float = 1e-2):
        """
        ...
        if self.merging_method_name == "average_merging":
            ...
            return Vector_unified, masks, rescales
        elif self.merging_method_name == "pave_ties_merging":
            merged_params = self.pave_ties_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                              param_value_mask_rate=param_value_mask_rate, scaling_coefficient=scaling_coefficient, trainers=trainers, num_ada_examples=num_ada_examples, rank_scale=rank_scale, reduce_non_diagonal_ratio=reduce_non_diagonal_ratio, model_name=model_name)
        elif self.merging_method_name == "pave_task_arithmetic":
            merged_params = self.pave_task_arithmetic(merged_model=merged_model, models_to_merge=models_to_merge, trainers=trainers, num_ada_examples=num_ada_examples,
                    exclude_param_names_regex=exclude_param_names_regex, rank_scale=rank_scale, reduce_non_diagonal_ratio=reduce_non_diagonal_ratio, model_name=model_name, scaling_coefficient=scaling_coefficient, regularization = regularization)    
            ...
            if mask_apply_method == "average_merging":
                ...
                merged_params = self.ties_merging(merged_model=merged_model, models_to_merge=new_models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                  param_value_mask_rate=param_value_mask_rate, scaling_coefficient=scaling_coefficient)
            elif mask_apply_method == "pave_ties_merging":
                merged_params = self.pave_ties_merging(merged_model=merged_model, models_to_merge=new_models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                              param_value_mask_rate=param_value_mask_rate, scaling_coefficient=scaling_coefficient, trainers=trainers, num_ada_examples=num_ada_examples, rank_scale=rank_scale, reduce_non_diagonal_ratio=reduce_non_diagonal_ratio, model_name=model_name)
            elif mask_apply_method == "pave_task_arithmetic":
                merged_params = self.pave_task_arithmetic(merged_model=merged_model, models_to_merge=new_models_to_merge, trainers=trainers, num_ada_examples=num_ada_examples,
                        exclude_param_names_regex=exclude_param_names_regex, rank_scale=rank_scale, reduce_non_diagonal_ratio=reduce_non_diagonal_ratio, model_name=model_name, scaling_coefficient=scaling_coefficient)    
            elif mask_apply_method == "emr_merging":# merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list
                Vector_unified, masks, rescales = self.emr_merging(merged_model=merged_model, models_to_merge=new_models_to_merge,
                                                exclude_param_names_regex=exclude_param_names_regex)
                return Vector_unified, masks, rescales
            else:
    ...
    def get_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
        ...
        # Return (Vector_unified, masks, rescales) whenever the selected merging method itself is EMR
        # or when EMR is selected as the mask apply method.
        if self.merging_method_name == "emr_merging" or mask_apply_method == "emr_merging":
            Vector_unified, masks, rescales = self.merging_models(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex, trainers=trainers,
                                                ...
                                                mask_apply_method=mask_apply_method, models_use_deepcopy=models_use_deepcopy, rank_scale=rank_scale, model_name=model_name, num_ada_examples=num_ada_examples)
            return Vector_unified, masks, rescales
        else:
            merged_params = self.merging_models(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex, trainers=trainers,
                                                ...
                                                nums_regmean_examples=nums_regmean_examples, reduce_non_diagonal_ratio=reduce_non_diagonal_ratio, param_value_mask_rate=param_value_mask_rate,
                                                weight_format=weight_format, weight_mask_rates=weight_mask_rates, use_weight_rescale=use_weight_rescale, mask_strategy=mask_strategy,
                                                mask_apply_method=mask_apply_method, models_use_deepcopy=models_use_deepcopy, rank_scale=rank_scale, model_name=model_name, num_ada_examples=num_ada_examples, regularization=regularization)
    ...
    def pave_ties_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, param_value_mask_rate: float = 0.8, scaling_coefficient: float = 1.0, trainers: list = None, num_ada_examples: list = None, rank_scale:list = None, reduce_non_diagonal_ratio: float = 1.0, model_name: str = None):
        """
        ties merging method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        ##########################################################
        ids = [1,2,3,4,5,6,7,8]
        # Use models_to_merge[ids] for rank search
        models_for_rank_search = [models_to_merge[i] for i in ids]
        trainers_for_rank_search = [trainers[i] for i in ids]
        num_ada_examples_for_rank_search = [num_ada_examples[i] for i in ids]
        
        ranks = rank_search(models_to_merge=models_for_rank_search, trainers=trainers_for_rank_search, model_name=model_name, num_ada_examples=num_ada_examples_for_rank_search, exclude_param_names_regex=exclude_param_names_regex, rank_ratio=rank_scale[1])
        
        ##########################################################
        ##########################################################
        # Create new_layer_ranks_by_model for IDs 1-8
        new_layer_ranks_by_model = defaultdict(dict)
        
        # First, collect all unique layer names from the rank search results
        unique_layer_names: set[str] = set()
        for model_id, layer_name, rank in ranks:
            unique_layer_names.add(layer_name)
        
        # Create a mapping from rank search model_id to actual model_id
        ...
        # Initialize all IDs 1-8 with corresponding ranks from their models
        for model_id in range(1, 9):
            for layer_name in unique_layer_names:
                if model_id < len(models_to_merge):
                    model = models_to_merge[model_id]
                    ...
                    for name, module in model.named_modules():
                        if name == layer_name and hasattr(module, 'weight'):
                            ...
                            new_layer_ranks_by_model[model_id][layer_name] = rank
                            break
        

        excluded_ranks_sum = 0
        for layer_name in unique_layer_names:
            if layer_name in new_layer_ranks_by_model[1]:
                # Check if this layer should be excluded based on exclude_param_names_regex
                should_exclude = any([re.match(exclude_pattern, layer_name) for exclude_pattern in exclude_param_names_regex])
                if not should_exclude:
                    excluded_ranks_sum += new_layer_ranks_by_model[1][layer_name]
        ...
        for rank_model_id, layer_name, rank in ranks:
            ...
            new_layer_ranks_by_model[actual_model_id][layer_name] = rank
        
        # Use the new structure
        layer_ranks_by_model = new_layer_ranks_by_model
        ...
        models_to_merge_task_vectors = [Ada_TaskVector_Cholesky(pretrained_model=merged_model,
                               finetuned_model=models_to_merge[i+1],
                               exclude_param_names_regex=exclude_param_names_regex,
                               trainer_pretrained=trainers[0],
                               trainer_finetuned=trainers[i+1],
                               num_examples=num_ada_examples[i+1],
                               reduce_non_diagonal_ratio=reduce_non_diagonal_ratio,
                               rank_scale=rank_ratio[i],
                               model_name=model_name
                               )
                for i in range(len(models_to_merge)-1)]
    ...
    def pave_task_arithmetic(self, merged_model: nn.Module, models_to_merge: list, trainers: list = None, num_ada_examples: list = None,
                    exclude_param_names_regex: list = None, rank_scale: list = None, reduce_non_diagonal_ratio: float = 1.0, model_name: str = None, scaling_coefficient: float = 1.0):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        rank_ratio = [rank_scale[0], rank_scale[1]]

        models_to_merge_task_vectors = []
        
        for i in range(len(models_to_merge)):
            print(f"Processing model {i+1}/{len(models_to_merge)} for task vector computation...")
            ...
            # Ensure models are on CPU before processing
            merged_model.to('cpu')
            models_to_merge[i].to('cpu')
            
            task_vector = Ada_TaskVector_Cholesky(pretrained_model=merged_model,
                               finetuned_model=models_to_merge[i],
                               exclude_param_names_regex=exclude_param_names_regex,
                               trainer_pretrained=trainers[0],
                               trainer_finetuned=trainers[i+1],
                               num_examples=num_ada_examples[i+1],
            ...
            models_to_merge_task_vectors.append(task_vector)
            
            # Force garbage collection and clear GPU memory after processing each model
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
            
            # Move models back to CPU to free any remaining GPU memory
            merged_model.to('cpu')
            models_to_merge[i].to('cpu')
        ...
        with torch.no_grad():
            ...
            print("Combining with pretrained model...")
            merged_params = merged_task_vector.combine_with_pretrained_model(pretrained_model=merged_model, scaling_coefficient=scaling_coefficient)
    ...
    def pave_emr_merging(self, merged_model: nn.Module, models_to_merge: list, trainers: list, num_ada_examples: list,
                    exclude_param_names_regex: list, rank_scale: list, reduce_non_diagonal_ratio: float = 1.0, model_name: str = None):
        merged_model.to('cpu')
        for model in models_to_merge:
            model.to('cpu')
        print("num_ada_examples type:", type(num_ada_examples))
        ...
        task_vectors = [Ada_TaskVector_Cholesky(pretrained_model=merged_model,
                               finetuned_model=models_to_merge[i+1],
                               exclude_param_names_regex=exclude_param_names_regex,
                               trainer_pretrained=trainers[0],
                               trainer_finetuned=trainers[i+1],
    ...
    def get_pave_emr_merged_model(
        self, 
        merged_model: nn.Module, 
        models_to_merge: list, 
        exclude_param_names_regex: list, 
        trainers: list,
        num_ada_examples: list,
        rank_scale: list,
        reduce_non_diagonal_ratio: float = 1.0
    ):
        """
        Get the merged model using pave_emr merging method
        
        :param merged_model: base model to merge with
        :param models_to_merge: list of models to merge
        :param exclude_param_names_regex: parameters to exclude from merging
        :param trainers: list of trainers (first one is for merged_model, rest for models_to_merge)
        :param num_ada_examples: list of integers, number of examples to use for each model
        :param reduce_non_diagonal_ratio: ratio to reduce non-diagonal elements
        :return: merged model parameters
        """
        try:
            if self.merging_method_name == "pave_emr_merging":
                print(f"Starting pave_emr_merging with {len(models_to_merge)} models and {len(trainers)} trainers")
                print(f"Number of examples per model: {num_ada_examples}")
                
                # Validate inputs
                if len(models_to_merge) != len(trainers):
                    print(f"Warning: Expected {len(models_to_merge)} trainers but got {len(trainers)}. First trainer should be for merged model.")
                
                if len(num_ada_examples) != len(trainers):
                    print(f"Warning: Expected {len(trainers)} elements in num_ada_examples but got {len(num_ada_examples)}.")
                    # Pad with default values if necessary
                    if len(num_ada_examples) < len(trainers):
                        num_ada_examples = num_ada_examples + [100] * (len(trainers) - len(num_ada_examples))
                        print(f"Padded num_ada_examples to: {num_ada_examples}")
                
                return self.pave_emr_merging(
                ...
                raise ValueError(f"Unknown merging method name: {self.merging_method_name}")
        except Exception as e:
            print(f"Error in get_pave_emr_merged_model: {str(e)}")
            import traceback
            traceback.print_exc()
            raise