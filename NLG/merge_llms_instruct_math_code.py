...
def apply_vector(vector, pretrained_model):#, scaling_coef=1.0):
...
def get_merge_performance(args: argparse.Namespace, finetuned_model_names: list, merge_task_names: list, models_to_merge: list, trainers: list, logger: logging.Logger,
    ...
    if merging_method.merging_method_name == "emr_merging" or args.mask_apply_method == "emr_merging":
        vector_unified, masks, rescales = merging_method.get_merged_model(merged_model=merged_model,
                                                    ...
                                                    num_ada_examples=None,
                                                    model_name=None,
                                                    models_use_deepcopy=False)
        for i, dataset in enumerate(merge_task_names):
            task_vector_recon = {}
            for n in vector_unified:
                task_vector_recon[n] =  vector_unified[n] * masks[n][i] * rescales[i]
            merged_model = apply_vector(task_vector_recon, merged_model)
    else:
    ...
    if args.merge_instruct:
        save_instruct_model_path = f"/ibex/user/anb/save_merge_models/{'_'.join(merge_task_names)}/instruct/{args.save_model_name}"
    if args.merge_math:
        save_math_model_path = f"/ibex/user/anb/save_merge_models/{'_'.join(merge_task_names)}/math/{args.save_model_name}"
    if args.merge_code:
        save_code_model_path = f"/ibex/user/anb/save_merge_models/{'_'.join(merge_task_names)}/code/{args.save_model_name}"
...
parser.add_argument("--merging_method_name", type=str, default="average_merging", help="name of the method to merge models",
                    choices=["average_merging", "task_arithmetic", "mask_merging", "emr_merging"])
...
parser.add_argument("--mask_apply_method", type=str, default="average_merging", help="merging method that the mask strategy applies",
                    choices=["average_merging", "task_arithmetic","emr_merging"])
...
if __name__ == "__main__":
    ...
    if args.merging_method_name == "average_merging":
    ...
    elif args.merging_method_name == "emr_merging":
        args.save_model_name = f"{args.merging_method_name}"
    else:
        ...
        elif args.mask_apply_method == "emr_merging":
            mask_apply_method_name = f"{args.mask_apply_method}"
        else:
    ...
    merging_method = MergingMethod(merging_method_name=args.merging_method_name, model_name=None)
