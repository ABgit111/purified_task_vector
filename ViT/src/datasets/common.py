...
def get_dataloader(dataset, is_train, args, image_encoder=None, seed=None):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(is_train, image_encoder, dataset, args.device)
        if seed is not None and is_train:
            generator = torch.Generator()
            generator.manual_seed(seed)
            dataloader = DataLoader(feature_dataset, batch_size=args.batch_size, shuffle=is_train, generator=generator)
        else:
            dataloader = DataLoader(feature_dataset, batch_size=args.batch_size, shuffle=is_train)
    else:
        if seed is not None and is_train:
            # Create a new dataloader with seeded generator for training data
            generator = torch.Generator()
            generator.manual_seed(seed)
            dataloader = DataLoader(dataset.train_dataset, batch_size=args.batch_size, shuffle=is_train, generator=generator)
        else:
