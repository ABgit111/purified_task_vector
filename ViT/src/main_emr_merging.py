...
model = 'ViT-B-16' # ViT-B-32 | ViT-B-16 | ViT-L-14
args = parse_arguments()
args.home = '/ibex/user/anb/Model_merging/EMR_Merging-main/merge_vit' # type your home path here
...
task_vectors = [
    TaskVector(pretrained_checkpoint, args.home + '/checkpoints/'+model+'/'+dataset_name+'/finetuned.pt', dataset_name = dataset_name) for dataset_name in exam_datasets
]

# merge models
vector_unified, masks, rescalers = emr_merge(task_vectors)
merge_elapsed_s = time.perf_counter() - merge_start
log.info(f'EMR merge time: {merge_elapsed_s:.3f} s')
